...
router.get("/current", async (req, res) => {
  try {
    ...
    if (friends.length === 0) return res.send(`No friends to show!`);
...
router.get("/pending", async (req, res) => {
  try {
    ...
    if (friends.length === 0) return res.send(`No pending friends to show!`);
    return res.send(friends);
  } catch (error) {
    return res.status(500).send(`Internal Server Error: ${error}`);
  }
...
router.get("/requests", async (req, res) => {
  try {
    ...
    if (friends.length === 0) return res.send(`No friend requests to show!`);
    return res.send(friends);
  } catch (error) {
    return res.status(500).send(`Internal Server Error: ${error}`);
  }
